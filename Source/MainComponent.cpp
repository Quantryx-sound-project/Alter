...
void MainComponent::PanelHost::resized()
{
    if (view)
    {
        // Get rotation angle from panel properties (0=0°, 1=90°, 2=180°, 3=270°)
        int rotationState = (int) panel.getProperty (AlterState::kRotationAngle, 0);
        rotationState = juce::jlimit (0, 3, rotationState);  // Clamp to 0-3

        auto bounds = getLocalBounds();

        // Check if OpenGL component (rotation doesn't work on OpenGL)
        bool isOpenGLComponent = (dynamic_cast<juce::OpenGLAppComponent*>(view.get()) != nullptr);

        if (rotationState == 0 || isOpenGLComponent)
        {
            // No rotation: view has same bounds as host
            view->setBounds (bounds);
            view->setTransform (juce::AffineTransform());
        }
        else if (rotationState == 1 || rotationState == 3)
        {
            // 90° or 270°: SWAP width ↔ height so rotated view fills entire space
            const int w = bounds.getWidth();
            const int h = bounds.getHeight();

            // View bounds: swap dimensions (height becomes new width)
            view->setBounds (0, 0, h, w);

            // Transform: translate view center to origin → rotate → translate to host center
            float angleRadians = (float) rotationState * juce::MathConstants<float>::halfPi;
            auto viewCentre = juce::Point<float> (h / 2.0f, w / 2.0f);  // center of swapped view
            auto hostCentre = bounds.getCentre().toFloat();

            auto transform = juce::AffineTransform()
                .translated (-viewCentre.x, -viewCentre.y)  // move view center to origin
                .rotated (angleRadians)                      // rotate around origin
                .translated (hostCentre.x, hostCentre.y);   // move to host center

            view->setTransform (transform);
        }
        else  // rotationState == 2 (180°)
        {
            // 180°: keep same dimensions, just rotate
            view->setBounds (bounds);
    ...
    if (dragOverlay)
    {
        ...
        dragOverlay->setTransform (juce::AffineTransform());  // Overlay always upright
    }
}
...
void MainComponent::PanelHost::paintOverChildren (juce::Graphics& g)
{
    ...
    {
        if (auto* meter = dynamic_cast<VisualAudioMeter*>(view.get()))
        {
            // Get mode name
            juce::String modeName;
            if (meter->getMeterMode() == VisualAudioMeter::MeterMode::RMS)
                modeName = "RMS";
            else if (meter->getMeterMode() == VisualAudioMeter::MeterMode::TruePeak)
                modeName = "Peak";
            else
                modeName = "LUFS";

            // Convert linear value to dB for display (uses smoothed value matching the bar)
            const float smoothedVal = meter->getSmoothedValue();
            float dbValue;

            if (meter->getMeterMode() == VisualAudioMeter::MeterMode::LUFS)
            {
                // LUFS: smoothed value is already in dB-like LUFS units
                dbValue = juce::jlimit (-60.0f, 0.0f, smoothedVal);
            }
            else
            {
                // RMS / True Peak: smoothed value is linear, convert to dB
                dbValue = (smoothedVal <= 0.00001f) ? -60.0f 
                        : juce::jlimit (-60.0f, 60.0f, 20.0f * std::log10 (smoothedVal));
            }

            // Format: "RMS: -12.3 dBFS" or "Peak: 0.0 dBFS" or "LUFS: -14.2 LUFS"
            if (meter->getMeterMode() == VisualAudioMeter::MeterMode::LUFS)
                label << modeName << ": " << juce::String (dbValue, 1) << " LUFS";
            else
                label << modeName << ": " << juce::String (dbValue, 1) << " dBFS";

            // Draw scale markings in HOST space for rotated meters (always readable)
            const int rotState = meter->getHostRotation();
            if (rotState == 1 || rotState == 3)
            {
                auto hostBounds = getLocalBounds().reduced (4);
                auto scaleInfo = meter->getScaleInfo();

                auto normalizeDb = [&scaleInfo](float db) -> float
                {
                    return juce::jlimit (0.0f, 1.0f, (db - scaleInfo.minDb) / (scaleInfo.maxDb - scaleInfo.minDb));
                };

                g.setFont (10.0f);

                for (float dbVal : scaleInfo.checkpoints)
                {
                    const float norm = normalizeDb (dbVal);

                    // For rotation 3 (270°, bottom-to-top): 0 dB at bottom, max at top
                    // For rotation 1 (90°, top-to-bottom): reversed
                    int y;
                    if (rotState == 3)
                        y = hostBounds.getBottom() - (int) (norm * hostBounds.getHeight());
                    else
                        y = hostBounds.getY() + (int) (norm * hostBounds.getHeight());

                    if (y >= hostBounds.getY() && y <= hostBounds.getBottom())
                    {
                        // Tick mark (horizontal line)
                        g.setColour (juce::Colours::lightgrey.withAlpha (0.6f));
                        const int tickW = (dbVal == 0.0f || dbVal == -60.0f) ? 12 : 6;
                        g.drawLine ((float) hostBounds.getX(), (float) y,
                                    (float) (hostBounds.getX() + tickW), (float) y, 1.0f);

                        // Label text (always horizontal, readable)
                        juce::String scaleLabel;
                        if (meter->getMeterMode() == VisualAudioMeter::MeterMode::LUFS)
                            scaleLabel = juce::String ((int) dbVal);
                        else
                            scaleLabel = (dbVal == 0.0f) ? "0" : juce::String ((int) dbVal);

                        g.setColour (juce::Colours::white.withAlpha (0.85f));
                        g.drawText (scaleLabel, hostBounds.getX() + tickW + 1, y - 6, 28, 12,
...
void MainComponent::applyPanelPropsToView (const juce::ValueTree& panel, juce::Component& view)
{
    ...
    if (type == "rms" || type == "audiometer")
    {
        ...
        // Apply meter mode (0=RMS, 1=True Peak, 2=LUFS)
        const int meterMode = (int) panel.getProperty (AlterState::kMeterMode, 0);
        if (meterMode == 0)
            v.setMeterMode (VisualAudioMeter::MeterMode::RMS);
        else if (meterMode == 1)
            v.setMeterMode (VisualAudioMeter::MeterMode::TruePeak);
        else
            v.setMeterMode (VisualAudioMeter::MeterMode::LUFS);

        const float smooth = (float) panel.getProperty (AlterState::kSmooth, 0.5f);
        v.setSmoothAmount (smooth);

        // Apply color mode (0=Standard, 1=Custom Gradient, 2=Custom Spectrum)
        ...
        if (colorModeInt == 1)
            colorMode = VisualAudioMeter::ColorMode::CustomGradient;
        else if (colorModeInt == 2)
            colorMode = VisualAudioMeter::ColorMode::CustomSpectrum;

        v.setColorMode (colorMode);

        // Apply bar color ONLY in custom modes (Standard always uses limegreen)
        if (colorMode != VisualAudioMeter::ColorMode::Standard && panel.hasProperty (AlterState::kColor))
        {
            ...
            v.setBarColour (juce::Colour ((uint32_t) argb));
        }
        else
        {
            // Standard mode: ALWAYS limegreen
            v.setBarColour (juce::Colours::limegreen);
        }

        // Tell meter about host rotation so it can skip text labels (PanelHost draws them)
    ...
    else if (type == "spectrum")
    {
        ...
        // panel-specific colour
        if (panel.hasProperty (AlterState::kColor))
        {
            const int argb = (int) panel.getProperty (AlterState::kColor);
            v.setLineColour (juce::Colour ((uint32_t) argb));
        }
    }
    else if (type == "oscillator" || type == "oscilator")
    {
        ...
        {
            const int argb = (int) panel.getProperty (AlterState::kColor);
            v.setColour (juce::Colour ((uint32_t) argb));
        }
        // sample rate (not all panels expose this property; ignore if absent)
        // smooth (common property)
        ...
        {
            const bool fill = (bool) panel.getProperty (AlterState::kNeon, false);
            v.setFill (fill);
        }
        // display mode
        ...
        {
            const float z = (float) panel.getProperty (AlterState::kZoom, 0.420f);
            v.setZoom (z);
        }
    }
    else if (type == "synesthesia")
    {
        ...
        {
            const bool sync = (bool) panel.getProperty (AlterState::kSyncBPM, true);
            v.setSyncToBPM (sync);
        }
