...
private:
    ...
    public:
        ContentPanel()
        {
            setSize(600, 1600);  // Tall content for scrolling (accounts for all sections + macOS fonts)
        }
        ...
        void paint(juce::Graphics& g) override
        {
            ...
            // MODE 1: RMS (Average)
            drawModeSection(g, area, "1. RMS MODE - AVERAGE ENERGY",
                "Root Mean Square measures the average energy of your audio signal.\n"
                "Think of it as a 'loudness thermometer' - shows overall signal level.",
                {
                    {"GREEN: Safe level (-inf to 0 dBFS)", juce::Colours::limegreen},
                    {"RED: Clipping (above 0 dBFS)", juce::Colours::red.withAlpha(0.85f)}
                },
                "USE CASES: General monitoring, mixing, DJing");
